-- Run after add_article_tags.sql.
...
-- merge "Trimester 1/2/3" into the canonical "1st/2nd/3rd Trimester" tags,
-- de-duping if an article already carries both forms
...
-- strip out the noisy/overly granular tags entirely (hospital names, lab
-- tests, single clinical terms). case-sensitive — 'antenatal care' here is
-- the lowercase variant specifically
update public.articles
set tags = (
...
-- tag, or 'General', if it was one of the removed noisy values
update public.articles
